...
export default function Nav({ isArrow = true, textColor = "", Linkfs = "" }) {
    ...
    const isMenuActive = (menuItem) => {
        ...
        const getSafeSegment = (url) => {
            if (typeof url !== "string") return "";
            const parts = url.split("/").filter(Boolean);
            return parts[0] || "";
        };

        const currentSegment = getSafeSegment(pathname);

        if (menuItem.href?.includes("/")) {
            if (getSafeSegment(menuItem.href) === currentSegment) {
                active = true;
            }
        }
        ...
        if (Array.isArray(menuItem) && menuItem.length) {
            active = menuItem.some(
                (elm) => getSafeSegment(elm?.href) === currentSegment
            );
        }
        ...
        if (Array.isArray(menuItem) && menuItem.length) {
            menuItem.forEach((item) => {
                item?.links?.forEach((elm2) => {
                    if (elm2.href?.includes("/")) {
                        if (getSafeSegment(elm2.href) === currentSegment) {
                            active = true;
                        }
                    }
                ...
                if (item.href?.includes("/")) {
                    if (getSafeSegment(item.href) === currentSegment) {
                        active = true;
                    }
                }
            });
                ...
                <div className="sub-menu submenu-default">
                    <ul className="menu-list">
                        {pages.map((item, index) => (
                            <li
                                key={index}
                                className={item.links ? "menu-item-2 " : ""}
                            >
                                {item.href.includes("#") ? (
                                    <a
                                        href={item.href}
