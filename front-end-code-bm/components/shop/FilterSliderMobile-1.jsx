...
const categories = [
  ...
  const filterSleeves = ["short", "long", "sleevesless"]; 
  const filterFbric = ["cotton", "wool", "silk", "leather"]; 
...
export default function FilterSliderMobile() {
    ...
    const handleSelectSleeves = (sleevesSize) => {
      if (selectedSleeves.includes(sleevesSize)) {
        setSelectedSleeves((pre) => [...pre.filter((el) => el != sleevesSize)]);
      } else {
        setSelectedSleeves((pre) => [...pre, sleevesSize]);
      }
    };
  
    const [selectedFabric, setSelectedFabric] = useState([]);
    const handleSelectFbric = (fabric) => {
      if (selectedFabric.includes(fabric)) {
        setSelectedFabric((pre) => [...pre.filter((el) => el != fabric)]);
      } else {
        setSelectedFabric((pre) => [...pre, fabric]);
      }
    };
    useEffect(() => {
        let filteredArrays = [];
        ...
        // if (selectedSleeves.length) {
        //   filteredArrays = [
        //     ...filteredArrays,
        //     [
        //       ...products1.filter((elm) =>
        //         elm.filterSleeves?.some((elm2) => selectedSleeves.includes(elm2))
        //       ),
        //     ],
        //   ];
        // }
        ...
        if (selectedSleeves.length) {
          filteredArrays = [
            ...filteredArrays,
            [...products1.filter((elm) => selectedSleeves.includes(elm.Sleeves))],
          ];
        }
        if (selectedFabric.length) {
          filteredArrays = [
            ...filteredArrays,
            [...products1.filter((elm) => selectedFabric.includes(elm.fabric))],
          ];
        }
          // console.log(filteredByselectedSizes);
  ...
  return (
  <>
      <div className="offcanvas offcanvas-start canvas-filter" id="mobFilterShop">
      ...
      <div className="widget-facet wd-categories">
        <div
          className="facet-title filter-element"
      ...
      <form action="#" id="facet-filter-form" className="facet-filter-form">
        ...
        <div className="widget-facet">
          ...
          <div id="Sleeves" className="collapse show">
            <ul className="tf-filter-group current-scrollbar mb_36">
              {filterSleeves.map((elm, i) => (
                <li
                  key={i}
                  onClick={() => handleSelectSleeves(elm)}
                  className="list-item d-flex gap-12 align-items-center"
                >
                  <input
                    type="radio"
                    className="tf-check tf-check-size"
                    readOnly
                    checked={selectedSleeves.includes(elm)}
                  />
                  <label className="label">
                    <span>{elm}</span>&nbsp;
                    <span>
                      (
                        {products1.filter((product) => product.Sleeves === elm).length}
                      )
                    </span>
                  </label>
                </li>
        ...
        <div className="widget-facet">
          ...
          <div id="fabric" className="collapse show">
            <ul className="tf-filter-group current-scrollbar mb_36">
              {filterFbric.map((elm, i) => (
                <li
                  key={i}
                  onClick={() => handleSelectFbric(elm)}
                  className="list-item d-flex gap-12 align-items-center"
                >
                  <input
                    type="radio"
                    className="tf-check tf-check-size"
                    readOnly
                    checked={selectedFabric.includes(elm)}
                  />
                  <label className="label">
                    <span>{elm}</span>&nbsp;
                    <span>
                      (
                        {products1.filter((product) => product.fabric === elm).length}
                      )
                    </span>
                  </label>
                </li>
