...
export default function Products({ magazine }) {
  return (
    <section className="our-previous-editions">
      <div className="container">
        ...
        <div className="hover-sw-nav hover-sw-2">
          ...
          >
            {magazine.map((product, index) => (
              <SwiperSlide key={index}>
                <a href={`${_url}/${product.pdf}`} target="_blank" className="blog-article-item w-100" download={`${_url}/${product.pdf.split('/').pop()}`}>
                  <div className="article-thumb radius-10">
                    <div className="hoverImg">
                      <Image
                        src={`${_url}/${product.image}`}
                        alt={product?.alt}
                        width={414}
                        height={517}
